...
const
  LISTENER_ID = '__EmitterListenerIDs',
  ONCE_ID     = '__EmitterListenersOnce';

/**
 * Class for implementation events
 *
 * @class Emitter
 */
class Emitter {
  ...
  _validateEvent (event) {
    if (typeof event !== 'string') {
      throw new Error(`As event expected type <string>, but has gotten: ${(typeof event)}.`);
    }
  }

  _validateListener (listener) {
    if (typeof listener !== 'function') {
      throw new Error(`As listener expected type <function>, but has gotten: ${(typeof listener)}.`);
    }
  }
  ...
  on(event, listener) {
    ...
    this._listener[event] === void 0 && (this._listener[event] = []);
    //Add id
    listener[LISTENER_ID] === void 0 && (listener[LISTENER_ID] = {});
    if (listener[LISTENER_ID][event] === void 0) {
      listener[LISTENER_ID][event] = this._id;
  ...
  once (event, listener) {
    let _listener = this.on(event, listener);
    if (typeof _listener === 'function') {
      listener[ONCE_ID] === void 0 && (listener[ONCE_ID] = {});
      listener[ONCE_ID][event] = true;
    }
  }
  ...
  emit (event) {
    let toRemove = [];
    this._validateEvent(event);
    this._listener[event] !== void 0 && (this._listener[event].forEach((handle) => {
      handle();
      if (handle[ONCE_ID] !== void 0 && handle[ONCE_ID][event]) {
  ...
  removeListener (event, listener) {
    this._validateEvent(event);
    this._validateListener(listener);
    if (typeof listener[LISTENER_ID] !== 'object' || listener[LISTENER_ID] !== null || listener[LISTENER_ID][event] === void 0) {
      return false;
    }
    let id = listener[LISTENER_ID][event];
    this._listener[event] instanceof Array && (this._listener[event] = this._listener[event].filter((listener) => {
      return listener[LISTENER_ID][event] !== id;
    }));
  }

  removeAllListeners (event) {
    this._validateEvent(event);
    this._listener[event] instanceof Array && this._listener[event].forEach((listener)=>{
      delete listener[LISTENER_ID][event];
    });
  ...
  listeners(event) {
    this._validateEvent(event);
    return this._listener[event] instanceof Array ? this._listener[event].filter(true) : [];
  }
